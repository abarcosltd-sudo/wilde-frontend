...
export const useWritingStore = create<WritingState>(set => ({
  ...
  updateTitle: title => set(s => ({
    currentWork: s.currentWork ? { ...s.currentWork, title } : null,
    isDirty: true,
  })),
  setCoverImage: url => set(s => ({
    currentWork: s.currentWork ? { ...s.currentWork, coverImageUrl: url } : null,
    isDirty: true,
  })),
  setCollaborators: ids => set(s => ({
    currentWork: s.currentWork ? { ...s.currentWork, collaborators: ids } : null,
  })),
